  ...
  const renderBooking = (booking, room) => {
    ...
    try {
      ...
      console.log('בדיקת תשלום וחוות דעת:', {
        ...
        hasInvoiceReceipt: booking.hasInvoiceReceipt, // 🆕 חשבונית אוטומטית מהמערכת
        manualInvoiceHandled: booking.manualInvoiceHandled, // 🆕 סימון ידני חשבונית
        ...
        >
          <Box sx={{ 
            ...
            <Box sx={{ 
              ...
              {/* אייקון חוות דעת - יוצג כאשר ההזמנה מ-Booking/Expedia הסתיימה ולא טופלה */}
              {needsReviewIcon && (
                <Tooltip title="נדרש מילוי חוות דעת" arrow placement="top">
                  <IconButton 
                    size="small" 
                    sx={{ 
                      p: 0,
